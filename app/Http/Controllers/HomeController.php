...
class HomeController extends Controller
{
    ...
    public function add_grades_cores(Request $request)
    {
        $old = StudentGrades::where('student_id', Auth::user()->id)
            ->where('cores_id', $request->cores_id)->get()->count();
        if ($old == 0) :
            StudentGrades::create($request->all());
        else :
            StudentGrades::where('student_id', Auth::user()->id)
                ->where('cores_id', $request->cores_id)->update($request->all());
        endif;
        return redirect()->back();
    }
    ...
    */
    public function end_student()
    {
        ...
        $text .="\n";
        $text .="سيتم تحويل اجابتك الي المرشد لأرشادك بشكل دقيق نتمني خلابل 24 ساعه نتمني لك التوفبق .";
        Mail::to(Auth::user()->email)->send(new SendAnswer( Auth::user()->email, Auth::user()->name,
    $text));
